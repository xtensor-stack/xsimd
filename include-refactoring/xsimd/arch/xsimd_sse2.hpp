...
namespace xsimd {
  ...
  namespace kernel {
    ...
    template<class A> batch<float, A> abs(batch<float, A> const& self, requires<sse2>) {
    ...
    // all
    template<class A> bool all(batch_bool<float, A> const& self, requires<sse2>) {
    ...
    // any
    template<class A> bool any(batch_bool<float, A> const& self, requires<sse2>) {
    ...
    // bitwise_and
    template<class A> batch<float, A> bitwise_and(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
      return _mm_and_ps(self, other);
    }
    template<class A> batch_bool<float, A> bitwise_and(batch_bool<float, A> const& self, batch_bool<float, A> const& other, requires<sse2>) {
      return _mm_and_ps(self, other);
    }
    template<class A, class T, class=typename std::enable_if<std::is_integral<T>::value, void>::type>
    ...
    // bitwise_andnot
    template<class A> batch<float, A> bitwise_andnot(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
      return _mm_andnot_ps(self, other);
    }

    template<class A> batch_bool<float, A> bitwise_andnot(batch_bool<float, A> const& self, batch_bool<float, A> const& other, requires<sse2>) {
      return _mm_andnot_ps(self, other);
    }
    template<class A, class T, class=typename std::enable_if<std::is_integral<T>::value, void>::type>
    ...
    // bitwise_not
    template<class A> batch<float, A> bitwise_not(batch<float, A> const& self, requires<sse2>) {
      return _mm_xor_ps(self, _mm_castsi128_ps(_mm_set1_epi32(-1)));
    }
    template<class A> batch_bool<float, A> bitwise_not(batch_bool<float, A> const& self, requires<sse2>) {
      return _mm_xor_ps(self, _mm_castsi128_ps(_mm_set1_epi32(-1)));
    }
    template<class A, class T, class=typename std::enable_if<std::is_integral<T>::value, void>::type>
    ...
    // bitwise_or
    template<class A> batch<float, A> bitwise_or(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
      return _mm_or_ps(self, other);
    }
    template<class A> batch_bool<float, A> bitwise_or(batch_bool<float, A> const& self, batch_bool<float, A> const& other, requires<sse2>) {
      return _mm_or_ps(self, other);
    }
    template<class A, class T, class=typename std::enable_if<std::is_integral<T>::value, void>::type>
    ...
    // bitwise_xor
    template<class A> batch<float, A> bitwise_xor(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
      return _mm_xor_ps(self, other);
    }
    template<class A> batch_bool<float, A> bitwise_xor(batch_bool<float, A> const& self, batch_bool<float, A> const& other, requires<sse2>) {
      return _mm_xor_ps(self, other);
    }
    template<class A, class T, class=typename std::enable_if<std::is_integral<T>::value, void>::type>
    ...
    // bitwise_cast
    template<class A, class T, class=typename std::enable_if<std::is_integral<T>::value, void>::type>
    batch<float, A> bitwise_cast(batch<T, A> const& self, batch<float, A> const &, requires<sse2>) {
    ...
    batch<Tp, A> bitwise_cast(batch<T, A> const& self, batch<Tp, A> const &, requires<sse2>) {
      return batch<Tp, A>(self.data);
    }
    template<class A, class T, class=typename std::enable_if<std::is_integral<T>::value, void>::type>
    batch<T, A> bitwise_cast(batch<float, A> const& self, batch<T, A> const &, requires<sse2>) {
    ...
    // bool_cast
    template<class A> batch_bool<int32_t, A> bool_cast(batch_bool<float, A> const& self, requires<sse2>) {
        return _mm_castps_si128(self);
    }
    template<class A> batch_bool<float, A> bool_cast(batch_bool<int32_t, A> const& self, requires<sse2>) {
        return _mm_castsi128_ps(self);
    }
    ...
    namespace detail
    {
      ...
      // complex_low
      template<class A> batch<float, A> complex_low(batch<std::complex<float>, A> const& self, requires<sse2>) {
        return _mm_unpacklo_ps(self.real(), self.imag());
      }
      // complex_high
      template<class A> batch<float, A> complex_high(batch<std::complex<float>, A> const& self, requires<sse2>) {
    ...
    // div
    template<class A> batch<float, A> div(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
    ...
    // eq
    template<class A> batch_bool<float, A> eq(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
      return _mm_cmpeq_ps(self, other);
    }
    template<class A> batch_bool<float, A> eq(batch_bool<float, A> const& self, batch_bool<float, A> const& other, requires<sse2>) {
    ...
    // ge
    template<class A> batch_bool<float, A> ge(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
    ...
    template<class A> batch_bool<float, A> gt(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
    ...
    // hadd
    template<class A> float hadd(batch<float, A> const& self, requires<sse2>) {
    ...
    {
    template<class A, class T, class=typename std::enable_if<std::is_integral<T>::value, void>::type>
    T hadd_default(batch<T, A> const& self, requires<sse2>) {
    ...
    // haddp
    template<class A> batch<float, A> haddp(batch<float, A> const* row, requires<sse2>) {
    ...
    // isnan
    template<class A> batch_bool<float, A> isnan(batch<float, A> const& self, requires<sse2>) {
    ...
    // load_aligned
    template<class A> batch<float, A> load_aligned(float const* mem, convert<float>, requires<sse2>) {
    ...
    // load_unaligned
    template<class A> batch<float, A> load_unaligned(float const* mem, convert<float>, requires<sse2>){
    ...
    namespace detail
    {
      ...
      template<class A> batch<std::complex<float>, A> load_complex(batch<float, A> const& hi, batch<float, A> const& lo, requires<sse2>) {
    ...
    // le
    template<class A> batch_bool<float, A> le(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
    ...
    // lt
    template<class A> batch_bool<float, A> lt(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
    ...
    // max
    template<class A> batch<float, A> max(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
    ...
    // min
    template<class A> batch<float, A> min(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
    ...
    // mul
    template<class A> batch<float, A> mul(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
    ...
    // neg
    template<class A, class T, class=typename std::enable_if<std::is_integral<T>::value, void>::type>
    batch<T, A> neg(batch<T, A> const& self, requires<sse2>) {
      return 0 - self;
    }
    template<class A> batch<float, A> neg(batch<float, A> const& self, requires<sse2>) {
    ...
    // neq
    template<class A> batch_bool<float, A> neq(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
      return _mm_cmpneq_ps(self, other);
    }
    template<class A, class T, class=typename std::enable_if<std::is_integral<T>::value, void>::type>
    batch_bool<T, A> neq(batch<T, A> const& self, batch<T, A> const& other, requires<sse2>) {
        return ~(self == other);
    }
    template<class A> batch_bool<float, A> neq(batch_bool<float, A> const& self, batch_bool<float, A> const& other, requires<sse2>) {
      return _mm_cmpneq_ps(self, other);
    }
    template<class A, class T, class=typename std::enable_if<std::is_integral<T>::value, void>::type>
    batch_bool<T, A> neq(batch_bool<T, A> const& self, batch_bool<T, A> const& other, requires<sse2>) {
        return ~(self == other);
    }
    ...
    // select
    template<class A> batch<float, A> select(batch_bool<float, A> const& cond, batch<float, A> const& true_br, batch<float, A> const& false_br, requires<sse2>) {
    ...
    // sqrt
    template<class A> batch<float, A> sqrt(batch<float, A> const& val, requires<sse2>) {
    ...
    // sadd
    template<class A> batch<float, A> sadd(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
    ...
    {
    template<class A, class T, class=typename std::enable_if<std::is_integral<T>::value, void>::type>
    batch<T, A> sadd_default(batch<T, A> const& self, batch<T, A> const& other, requires<sse2>) {
      if(std::is_signed<T>::value) {
    ...
    batch<float, A> set(batch<float, A> const&, requires<sse2>, Values... values) {
    ...
    // ssub
    template<class A> batch<float, A> ssub(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
    ...
    {
    template<class A, class T, class=typename std::enable_if<std::is_integral<T>::value, void>::type>
    batch<T, A> ssub_default(batch<T, A> const& self, batch<T, A> const& other, requires<sse2>) {
      if(std::is_signed<T>::value) {
    ...
    // store_aligned
    template<class A> void store_aligned(float *mem, batch<float, A> const& self, requires<sse2>) {
    ...
    // store_unaligned
    template<class A> void store_unaligned(float *mem, batch<float, A> const& self, requires<sse2>) {
    ...
    // zip_hi
    template<class A> batch<float, A> zip_hi(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
    ...
    // zip_lo
    template<class A> batch<float, A> zip_lo(batch<float, A> const& self, batch<float, A> const& other, requires<sse2>) {
