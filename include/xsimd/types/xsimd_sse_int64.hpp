    ...
    namespace detail
    {
            ...
            : sse_int_kernel_base<batch<T, 2>>
        {
            ...
            static batch_type sadd(const batch_type& lhs, const batch_type& rhs)
            {
                ...
                */
                ...
                batch<uint64_t, 2> ux = (batch<uint64_t, 2>)lhs;
                const batch<uint64_t, 2> uy = (batch<uint64_t, 2>)rhs;
                const batch<uint64_t, 2> res = _mm_add_epi64(ux, uy);
                const batch<uint64_t, 2> vmax = _mm_set1_epi64x(std::numeric_limits<int64_t>::max());
                const batch<uint64_t, 2> shr = _mm_srl_epi64(ux, _mm_set1_epi32(sizeof(int64_t) * std::numeric_limits<unsigned char>::digits));
                ux = _mm_add_epi64(shr, vmax);

                const batch<uint64_t, 2> xor_ux_uy = _mm_xor_si128(ux, uy);
                const batch<uint64_t, 2> xor_uy_res = _mm_xor_si128(uy, res);
                const batch<uint64_t, 2> not_xor_uy_res = _mm_andnot_si128(xor_uy_res, _mm_set1_epi8(-1));

                const batch<uint64_t, 2> u_orb = _mm_or_si128(xor_ux_uy, not_xor_uy_res);
                const batch_type i_orb = (batch_type)u_orb;

                const batch_type zeros = _mm_set1_epi64x(0);
#if XSIMD_X86_INSTR_SET >= XSIMD_X86_SSE4_1_VERSION
                __m128i gteq_to_zero = _mm_andnot_si128(_mm_cmpgt_epi64(zeros, i_orb), _mm_set1_epi8(-1));
#else
               
                const auto tmp = _mm_sub_epi64(_mm_setzero_si128(), _mm_srli_epi64(_mm_sub_epi64(i_orb, zeros), 63));
                __m128i gteq_to_zero = _mm_andnot_si128(tmp, _mm_set1_epi8(-1));
#endif
            ...
            : sse_int64_batch_kernel<uint64_t>
        {
            ...
            static batch_type sadd(const batch_type& lhs, const batch_type& rhs)
            {
                ...
                * ====================================================
                */
                const auto ures = _mm_add_epi64(lhs, rhs);
                const auto umax = _mm_set1_epi64x(std::numeric_limits<uint64_t>::max());
#if XSIMD_X86_INSTR_SET >= XSIMD_X86_SSE4_1_VERSION
                const auto is_overflow=_mm_cmpgt_epi64(_mm_add_epi64(lhs, umax), _mm_add_epi64(ures, umax));
                return _mm_blendv_epi8(ures,umax,is_overflow);
#else
                const auto is_overflow=_mm_sub_epi64(_mm_setzero_si128(),_mm_srli_epi64(_mm_sub_epi64(_mm_add_epi64(ures, umax), _mm_add_epi64(lhs, umax)), 63));
            ...
            static batch_type ssub(const batch_type& lhs, const batch_type& rhs)
            {
                ...
                */
                ...
                const auto ures = _mm_sub_epi64(lhs, rhs);
                const auto cte = _mm_set1_epi64x(std::numeric_limits<uint64_t>::max());
                const auto umin = _mm_set1_epi64x(std::numeric_limits<uint64_t>::lowest());
#if XSIMD_X86_INSTR_SET >= XSIMD_X86_SSE4_1_VERSION
                const auto is_underflow=_mm_cmpgt_epi64(_mm_add_epi64(rhs, cte), _mm_add_epi64(lhs, cte));
                return _mm_blendv_epi8(ures,umin,is_underflow);
#else
                const auto is_underflow = _mm_sub_epi64(_mm_setzero_si128(), _mm_srli_epi64(_mm_sub_epi64( _mm_add_epi64(lhs, cte), _mm_add_epi64(rhs, cte)), 63));
