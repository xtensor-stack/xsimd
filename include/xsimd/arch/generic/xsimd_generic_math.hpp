...
namespace xsimd
{
    ...
    namespace kernel
    {
        ...
        // hadd
        template <class A, class T>
        XSIMD_INLINE std::complex<T> hadd(batch<std::complex<T>, A> const& self, requires_arch<generic>) noexcept
        {
        ...
        namespace detail
        {
            ...
            struct lgamma_impl<batch<double, A>>
            {
                ...
                // This call is recursive, we cannot flag it as XSIMD_INLINE
            ...
            private:
                static XSIMD_INLINE batch_type large_negative(const batch_type& q) noexcept
                {
                    batch_type w = lgamma_impl<batch_type>::compute(q);
                    batch_type p = floor(q);
                    batch_type z = q - p;
                    auto test2 = (z < batch_type(0.5));
                    z = select(test2, z - batch_type(1.), z);
