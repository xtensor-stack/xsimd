...
namespace xsimd
{
    namespace kernel
    {
        ...
        XSIMD_INLINE batch<T, A> load_masked(T const* mem, batch_bool_constant<T, A, Values...> mask, convert<T>, Mode, requires_arch<avx>) noexcept
        {
            ...
            {
                constexpr auto mlo = ::xsimd::detail::lower_half<avx_128>(batch_bool_constant<int_t, A, Values...> {});
            ...
            {
                constexpr auto mhi = ::xsimd::detail::upper_half<avx_128>(mask);
        ...
        XSIMD_INLINE void store_masked(T* mem, batch<T, A> const& src, batch_bool_constant<T, A, Values...> mask, Mode, requires_arch<avx>) noexcept
        {
            ...
            XSIMD_IF_CONSTEXPR(mask.countl_zero() >= half_size)
            {
                constexpr auto mlo = ::xsimd::detail::lower_half<avx_128>(mask);
                const batch<T, avx_128> lo = detail::lower_half(src);
                store_masked<avx_128>(mem, lo, mlo, Mode {}, avx_128 {});
            }
            // upper 128-bit half
            else XSIMD_IF_CONSTEXPR(mask.countr_zero() >= half_size)
            {
                constexpr auto mhi = ::xsimd::detail::upper_half<avx_128>(mask);
                const batch<T, avx_128> hi = detail::upper_half(src);
                store_masked<avx_128>(mem + half_size, hi, mhi, Mode {}, avx_128 {});
            }
            else
            {
